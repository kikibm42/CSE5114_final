...
def fetch_live_games() -> pd.DataFrame:
    df = run_live("""
        ...
        FROM LIVE_DATA
        WHERE processed_time >= DATEADD(hour, -6, CURRENT_TIMESTAMP())
        QUALIFY ROW_NUMBER() OVER (PARTITION BY game_id ORDER BY processed_time DESC) = 1
    """)
    if df.empty:
        return df
    mask = df["game_status"].str.strip().isin(["2"]) | df["game_status"].str.strip().str.match(r"^(Q[1-4]|OT)", na=False)
...
def load_roster_stats(team_id: int, season: str) -> pd.DataFrame:
    return run_hist(
        ...
        SELECT
            ...
            ROUND(AVG(
                fgps.pts
                + 0.4 * fgps.fgm  - 0.7 * fgps.fga
                - 0.4 * (fgps.fta - fgps.ftm)
                + 0.7 * fgps.oreb + 0.3 * fgps.dreb
                + fgps.stl + 0.7 * fgps.ast + 0.7 * fgps.blk
                - 0.4 * fgps.pf   - fgps.tov
            ), 2)               AS game_score
...
def render_roster(df: pd.DataFrame) -> None:
    ...
    for col in ["PPG", "RPG", "APG", "SPG", "BPG", "Game Score"]:
...
def main():
    ...
    st.markdown(
        '<div class="section-title">🟢 Live Games Right Now</div>',
        unsafe_allow_html=True,
    )
    ...
    if live_error:
        ...
        if chosen_label == "— pick a game —":
            ...
            selected_game_id = None
            st.info(
                "Pick a game from the dropdown above to see live progression and historical data."
            )
        else:
        ...
        if hist_ok:
            ...
            for col, t_id, t_name, t_code in [
                ...
                with col:
                    ...
                    avg_fg_pct = (
                        game_stats["fg_pct"].mean() if not game_stats.empty else 0.0
                    )
            ...
            if h_id and a_id:
                st.markdown("---")
                st.markdown(
                    f'<div class="section-title">🆚 Head-to-Head — {h_name} vs {a_name}</div>',
                    unsafe_allow_html=True,
                )
